    ...
    export default {
        ...
        mounted() {
            ...
            load().then(res => {
                ...
                Loading();
                //初始化每题的答题数据
                console.log("选择模板。。。。。",question.dragShow);

                if(question.dragShow == '2'){
                    question.sources.map((item,index)=>{
                        item.content.option2.blank.map((mitem,mindex)=>{
                            mitem.image.keys.map((kitem,kindex)=>{
                                // console.log("打印....",kitem.value,item.rightKey)
                                if(kindex == 0){
                                    item.rightKey.push(kitem.value)
                                }
                            })
                        })
                    })
                }else if(question.dragShow == '1'){
                    question.sources.map((item,index)=>{
                        item.content.option1.blank_drag.map((mitem,mindex)=>{
                            mitem.image.keys.map((kitem,kindex)=>{
                                // console.log("打印....",kitem.value,item.rightKey)
                                if(kindex == 0){
                                    item.rightKey.push(kitem.value)
                ...
                answer.answerDefault({type:2,useranswer:[],times:[]});
